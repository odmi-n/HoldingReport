...
# utils ディレクトリを import 可能にする
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..')))
try:
    from src.utils.db import get_latest_companies_by_date
except ImportError:
    logging.error("❌ src/utils/db.py が見つかりません")
    # エラーでも続行する（他の機能は使える）

# 株みくじデータを読み込む
def load_stock_fortune_data():
    try:
        # 現在のファイルからの相対パスで株みくじデータを読み込む
        current_dir = os.path.dirname(os.path.abspath(__file__))
        fortune_file = os.path.join(current_dir, 'stock_fortune.json')
